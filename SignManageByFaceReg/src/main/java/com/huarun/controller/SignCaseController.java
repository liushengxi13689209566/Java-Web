        ...
        for (int i = 0; i < courseIDList.size(); i++) {
            ...
            for (int j = 0; j < majorClassIDList.size(); j++) {
                ...
                if (majorClassIDList.get(j).getClass_id() == 0) {
                    List<MajorClassDO> classIDList =
                            majorClassService.getClassListByMajorID(majorClassIDList.get(j).getMajor_id());
                    for (MajorClassDO majorClassDO : classIDList) {
                        rows.addAll(courseSignCaseRecordService.getCourseSignCaseRecord());
                    }
                } else {
                    rows.addAll()
                }
